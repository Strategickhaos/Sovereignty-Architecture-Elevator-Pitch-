 ...
 */
export function binaryToTriary(binary) {
    const trinary = [];
 ...
 */
function convertPairToTriBit(pair, position) {
 ...
 */
export function calculateWaveValue(tribit, time, frequency) {
 ...
 */
export function encodeToTriWave(data) {
 ...
 */
export function decodeFromTriWave(tribits) {
    let binary = '';
 ...
 */
export function createSuperposition(tribits) {
    return {
        tribits,
        collapsed: false,
 ...
 */
export function collapseSuperposition(state, time) {
    if (!state.collapsed) {
 ...
 */
export function generateSovereignBinary(input, frequency = 30000) {
    ...
    for (let i = 0; i < samples; i++) {
        ...
        const waveSum = tribits.reduce((sum, tb) => sum + calculateWaveValue(tb, time, frequency), 0);
