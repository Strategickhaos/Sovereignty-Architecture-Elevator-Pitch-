 ...
 */
export const SOVEREIGN_ELEMENTS = [
    {
        number: 1,
        symbol: 'H',
        name: 'Hydrogenium',
 ...
 */
export function getElementByNumber(number) {
    return SOVEREIGN_ELEMENTS.find(e => e.number === number);
}
/**
 * Get elements by operation type
 */
export function getElementsByOperation(operation) {
    return SOVEREIGN_ELEMENTS.filter(e => e.operation.toLowerCase().includes(operation.toLowerCase()));
}
/**
 * Get elements by frequency range
 */
export function getElementsByFrequencyRange(minHz, maxHz) {
    return SOVEREIGN_ELEMENTS.filter(e => e.frequency >= minHz && e.frequency <= maxHz);
}
/**
 * Generate element for custom operation
 */
export function createCustomElement(number, symbol, name, operation, frequency) {
    return {
        number,
        symbol,
        name,
