 ...
 */
import { demystify, batchDemystify } from './demystifier.js';
// Simple test runner
...
function testLinguisticLayer() {
    ...
    assert(result.layer_1_semantic.includes("gas_dynamics") && result.layer_1_semantic.includes("joules_per_second"), "Converts 'spiritual' and 'energy' to mechanical terms");
}
function testNumericLayer() {
    console.log('\n🔥 Testing Layer 2: Numeric Encoding');
    const result = demystify("I am a chosen one");
    assert(result.layer_2_numeric.length > 0, "Generates numeric codes for grounding operations");
    assert(result.layer_2_numeric.includes(0x4E4F4944), // NO_IDENTITY
...
function testWaveLayer() {
    ...
    assert(result.layer_3_wave.frequency > 0 && result.layer_3_wave.frequency <= 100, "Generates stable low-frequency signal");
    assert(result.layer_3_wave.waveform === "square", "Uses square waveform for digital clarity");
    assert(result.layer_3_wave.SNR === "high", "Achieves high signal-to-noise ratio");
}
function testDNALayer() {
    console.log('\n🔥 Testing Layer 4: DNA Codon Encoding');
    const result = demystify("I channel vibrations");
    assert(result.layer_4_dna.length === 7, "Generates complete 7-codon sequence");
    assert(result.layer_4_dna[0] === "AUG", "Starts with START codon (AUG)");
    assert(result.layer_4_dna[6] === "UAA", "Ends with STOP codon (UAA)");
}
function testValidationLayer() {
    console.log('\n🔥 Testing Layer 5: Grounding Validation');
    const result = demystify("I can measure frequency at 432 Hz");
    assert(result.layer_5_validation.checks.length === 6, "Performs all 6 grounding checks");
    const measurableCheck = result.layer_5_validation.checks.find(c => c.check === "MEASURABLE");
    assert(measurableCheck !== undefined && measurableCheck.passed, "Passes MEASURABLE check for statements with numbers");
}
function testDemystificationTable() {
    ...
    testCases.forEach(test => {
        ...
        assert(result.final_output.includes(test.expected.toLowerCase()) ||
            result.final_output.toLowerCase().includes(test.expected.toLowerCase()), `Correctly demystifies: "${test.input}"`);
    });
}
function testIsGroundedFunction() {
    ...
    groundedExamples.forEach(example => {
        ...
        assert(result.layer_5_validation.checks.some(c => c.passed), `Processes grounded statement: "${example}"`);
    });
}
function testBatchDemystify() {
    ...
    assert(results.length === inputs.length, `Processes all ${inputs.length} inputs in batch`);
    results.forEach((result, index) => {
        assert(result.original === inputs[index], `Preserves original input for batch item ${index + 1}`);
    });
}
function testMaximumFuckItEnergy() {
    ...
    assert(result !== null && result.final_output !== "", "Processes the maximum fuck-it energy statement");
