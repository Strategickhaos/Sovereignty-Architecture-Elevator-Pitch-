...
function printNodeDetails(nodeId: string, node: any) {
  console.log(`
┌─────────────────────────────────────────────────────────┐
...
🛠️  Tools:
${node.tools.map((t: string) => `   • ${t}`).join('\n')}

🎯 Triggers:
${node.triggers.map((t: string) => `   • ${t}`).join('\n')}

📊 Expected Output:
${node.expected_output.map((o: string) => `   • ${o}`).join('\n')}
`);
}
...
function handleTool(args: string[]) {
...
function handleNode(args: string[]) {
  const nodeId = args[0];
  if (!nodeId) {
...
function handleNext(args: string[]) {
  const nodeId = args[0];
  if (!nodeId) {
    console.log('❌ Error: Please provide a node ID');
    return;
...
function handlePath(args: string[]) {
  if (args.length < 2) {
