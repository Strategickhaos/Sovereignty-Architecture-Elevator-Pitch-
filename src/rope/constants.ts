...
export class RopeConstants {
  private data: RopeConstantsData;
  ...
  constructor(dataPath?: string) {
    ...
    try {
      ...
      this.data = JSON.parse(content) as RopeConstantsData;
    } catch (error) {
   ...
   */
  getConstant(key: string): RopeConstant | undefined {
   ...
   */
  getConstantsByCategory(category: string): RopeConstant[] {
   ...
   */
  searchConstants(query: string): ConstantQueryResult[] {
    const lowerQuery = query.toLowerCase();
    const results: ConstantQueryResult[] = [];
   ...
   */
  getConstantsByJurisdiction(jurisdiction: string): RopeConstant[] {
    return this.data.constants.filter(c => 
   ...
   */
  getConstantsByConfidence(confidence: 'low' | 'medium' | 'high'): RopeConstant[] {
   ...
   */
  private evaluateConstraint(constraint: string, value: number): boolean {
    try {
   ...
   * Validate a constant
   */
  validateConstant(key: string): ValidationResult {
   ...
   */
  knotEfficiencyInBounds(value: number): boolean {
   ...
   */
  pulleyEfficiencyInBounds(value: number): boolean {
   ...
   */
  designFactorMinimum(value: number, minimum: number = 3.0): boolean {
   ...
   */
  fallFactorInBounds(value: number): boolean {
   ...
   */
  bridleAngleWarning(value: number): boolean {
    return value <= 120.0;
  }
}
 ...
 */
let defaultInstance: RopeConstants | null = null;

/**
 * Get the default rope constants instance
 */
export function getRopeConstants(dataPath?: string): RopeConstants {
