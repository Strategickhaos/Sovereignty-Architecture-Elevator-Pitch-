  ...
  try {
    ...
    switch (name) {
      ...
      case 'deploy': {
        ...
        const deployChannel = getChannelId(
          config.refinory?.discord?.announce_channel || '#deployments'
        )
        if (deployChannel) {
      ...
      case 'refinory-status': {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields([
            ...
            {
              name: 'Experts Active',
              value: status.active_experts?.join(', ') || 'None',
              inline: true,
            },
          ])
        ...
        if (status.artifacts?.length) {
          embed.addFields([{ name: 'Artifacts', value: status.artifacts.slice(0, 5).join('\n') }])
        }
